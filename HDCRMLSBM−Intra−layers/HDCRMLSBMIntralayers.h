...
double JointEntropy();
double Apply_method_Restricted1();
...
{
  unsigned int i,j,m;//-Wunused, j, k;
  unsigned int l;
  double ratiovalue = 0;
  int fromcount = 0;
  int destcount = 0;
  ...
  for (m=0 ; m<Layers ; m++)
      ...
      for (i=0 ; i<MaxComms ; i++)
      {
         CurrentCommVerticesNew[i] += CurrentCommVertices[m][i];
         CurrentCommStubsNew[i] += (double(CurrentCommStubs[m][m][i])/LL[m][m]);
         for (j=0 ; j<MaxComms ; j++)
             CurrentEdgeMatrixNew[i][j] += (double(CurrentEdgeMatrix[m][m][i][j])/LL[m][m]);
      }
                 ...
                 if (NeighborLayers[ fromLayer] == 1)
                 {
                    ...
                    for (i=0 ; i<ActualDiffComms[fromLayer] ; i++)
                    {
                           ...
                           if ((NeighborSet[ fromLayer ] [0 ][ i] != azz) && (NeighborSet[ fromLayer][0 ][i] != destination))
                           {
                                    ...
                                    help1 = double(CurrentEdgeMatrixNew[azz ][NeighborSet[fromLayer ][0 ][i]]);
                                    help2 = double(CurrentEdgeMatrixNew[destination ][NeighborSet[fromLayer ][0 ][i]]);
                                    help3 = double(NeighborSet[fromLayer ][1 ][i]);

                                    //cout<<"kabiri1"<<endl;
                                    ratiovalue = ratiovalue + (LogFunction(help1-(help3/(LL[fromLayer ][fromLayer ]))) - LogFunction(help1 ));
                                    ratiovalue = ratiovalue + (LogFunction(help2+(help3/(LL[fromLayer ][fromLayer ]))) - LogFunction(help2 ));
                                    
                                    help1 = double(CurrentEdgeMatrixNew[NeighborSet[fromLayer ][0 ][i]][azz ]);
                                    help2 = double(CurrentEdgeMatrixNew[NeighborSet[fromLayer ][0 ][i]][destination ]);
                                    help3 = double(NeighborSet[fromLayer ][1 ][i]);
                        ...
                        //cout<<"kabiri3"<<endl;
                         ...
                         help2 = double(fromcount - destcount);
                         ratiovalue = ratiovalue + LogFunction(help1 + (help2/(LL[fromLayer ][fromLayer ])) ) - LogFunction(help1 );
